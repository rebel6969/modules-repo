...
usage:- .git reply_to_any_plugin //can be any type of file too. but for plugin must be in .py 
...
async def download(event):
	...
	if not reponame and Config.GIT_REPO_NAME is None:
		await event.edit("`Please ADD a proper repo name from Vars or specify your repo name as in the example: .git <reponame>`")
		return
	if event.fwd_from:
		return	
...
async def git_commit(file_name,mone):
	...
	if create_file == True:
		...
		try:
			...
			for f in files:
				os.remove(f)
			link = "/" + reponame + "/blob/master/" + file_name
			rawlink = "/" + reponame + "/master/" + file_name
