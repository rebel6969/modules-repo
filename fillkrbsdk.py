...
class Config:
    REPO_NAME = "YHA DAAL APNA LUND"
    GITHUB_ACCESS_TOKEN = "fill kr ye bsdk"
    GIT_NAME = "bsdk krna fill"
    GITHUB_ACCESS_TOKEN = "fill ni kia to lund ni"
...
async def download(event):
	...
	if not reponame and Config.REPO_NAME is None:
...
async def git_commit(file_name,mone):
	...
	if Config.REPO_NAME is not None:
		repo = g.get_repo(Config.GIT_NAME + "/" + reponame)
	else:
		repo = g.get_repo(Config.GIT_NAME + "/" + reponame)
	print(repo.name)
	create_file = True
	...
	if create_file == True:
		...
		try:
			...
			for f in files:
				os.remove(f)
			link =  "https://github.com/" + Config.GIT_NAME + "/" + reponame + "/blob/master/" + file_name
			rawlink = "https://raw.githubusercontent.com/" + Config.GIT_NAME + "/" + reponame + "/master/" + file_name
