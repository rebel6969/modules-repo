...
if [ ! x"" = x"$DYNO" ] && ! command -v python >/dev/null; then
  ...
  runout git clone https://github.com/opendoor-labs/heroku-buildpack-p7zip || { endspin "Bootstrap download failed!"; exit 1; }
  runout git clone https://github.com/amivin/aria2-heroku || { endspin "Bootstrap download failed!"; exit 1; }
  runout git clone https://github.com/heroku/heroku-buildpack-google-chrome || { endspin "Bootstrap download failed!"; exit 1; }
  runout git clone https://github.com/heroku/heroku-buildpack-chromedriver || { endspin "Bootstrap download failed!"; exit 1; }
  runout git clone https://github.com/ewwink/heroku-google-drive.git || { endspin "Bootstrap download failed!"; exit 1; }
  runout git clone https://github.com/emartech/heroku-buildpack-cloud-sql-proxy.git || { endspin "Bootstrap download failed!"; exit 1; }
  runout git clone https://github.com/jonathanong/heroku-buildpack-ffmpeg-latest.git|| { endspin "Bootstrap download failed!"; exit 1; }
  runout git clone https://github.com/rebel6969/heroku-buildpack-youtube-dl.git || { endspin "Bootstrap download failed!"; exit 1; }
